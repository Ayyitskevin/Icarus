...
import {
  computeGate2RepairAEvidenceDigest,
  GATE2_REPAIR_A_COHORT_LIMITATIONS,
  GATE2_REPAIR_A_ORACLES,
  GATE2_REPAIR_A_PROVIDER,
  parseAndValidateGate2RepairACohortResult,
  validateGate2RepairACohortResult,
} from "./gate2-repair-cohort-a-contract.mjs";
...
const reportPath = path.join(reportDirectory, "gate2-repair-cohort-a-report.json");
const PYTHON_IMAGE =
...
function operatorTargets(benchmarkCase, repository) {
  ...
  assertCondition(targets.length > 0, `${benchmarkCase.id} has no expected repair target`);
  assertCondition(
    targets.every((target) => sourcePaths.has(target)),
    `${benchmarkCase.id} repair cohort A requires existing targets only`,
  );
...
function planResponse(benchmarkCase, repository, oracle) {
  ...
  return {
    summary: "Apply the bounded Gate 2 repair fixture.",
    steps: ["Apply only the approved repair", "Run the registered scenario check"],
...
async function patchResponse(benchmarkCase, repository, oracle) {
  ...
  return {
    summary: "Apply only the pinned Gate 2 repair.",
    edits: oracle.approvedFiles.map((file) => {
      assertCondition(file.op === "modify", `${benchmarkCase.id} admits modify repairs only`);
      ...
      return {
        ...
        content: null,
        rationale: "Apply only the manifest-approved repair bytes.",
      };
    }),
  };
}

async function providerQueue(repairCases, repositories) {
  const queue = [];
  for (const [index, benchmarkCase] of repairCases.entries()) {
  ...
  for (const [index, request] of requests.entries()) {
    ...
    assertCondition(
      request.caseId === benchmarkCase.id &&
        ...
        body.model === GATE2_REPAIR_A_PROVIDER.model &&
        body.stream === false &&
        body.think === false &&
...
async function evaluateCase({
  ...
  assertCondition(
    recall === 1 && digestProvenanceCoverage === 1,
    `${benchmarkCase.id} retrieval evidence is incomplete`,
  );
  ...
  const providerConfig = createProviderConfig({
    kind: "ollama",
    model: GATE2_REPAIR_A_PROVIDER.model,
  ...
  assertCondition(
    ...
    `${benchmarkCase.id} did not produce exact passing private-workspace evidence`,
  );
  const finalFiles = [];
  ...
  observation.scenarioEvidenceSha256 = computeGate2RepairAEvidenceDigest(observation);
  return observation;
}
...
async function prepareReportDestination() {
  ...
  if (existing !== null) {
    assertCondition(
      ...
      "existing Gate 2 repair report is unsafe",
    );
...
async function persistReport(report, manifest, manifestSha256) {
  ...
  validateGate2RepairACohortResult(report, manifest, manifestSha256);
  const temporaryPath = path.join(
    reportDirectory,
    `.gate2-repair-cohort-a-${process.pid}-${randomUUID()}`,
  );
  ...
  assertCondition(
    ...
    "persisted Gate 2 repair report changed bytes",
  );
  parseAndValidateGate2RepairACohortResult(persisted.toString("utf8"), manifest, manifestSha256);
}

async function main() {
  assertCondition(process.argv.length === 2, "usage: node scripts/gate2-repair-cohort-a.mjs");
  ...
  const repairCases = loaded.manifest.cases.filter((entry) => repairCaseIds.has(entry.id));
  assertCondition(
    JSON.stringify(repairCases.map((entry) => entry.id)) ===
      JSON.stringify(GATE2_REPAIR_A_ORACLES.map((entry) => entry.caseId)),
    "manifest repair cohort A changed from the evaluator registry",
  );
  const repositories = new Map(
    loaded.manifest.repositories.map((repository) => [repository.id, repository]),
  );
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "icarus-gate2-repair-"));
  const providerServer = await startLoopbackProvider(
    await providerQueue(repairCases, repositories),
  );
  ...
  globalThis.fetch = async (input, init) => {
    ...
    if (
      ...
      throw new Error(`Gate 2 repair blocked non-loopback fetch: ${target.origin}`);
    }
  ...
  try {
    ...
    assertCondition(
      providerServer.requests.length === 10 &&
        providerServer.remaining() === 0 &&
        fetchEffects.loopbackProviderRequests === 10 &&
    ...
    const sandboxCheckExecutions = observations.reduce(
      (count, observation) =>
        count + observation.baseline.checks.length + observation.mutation.checks.length,
      0,
    );
    assertCondition(
      sandboxCheckExecutions === observations.length * 2,
      "repair cohort A sandbox check evidence changed",
    );
    const report = {
      schemaVersion: 1,
      ...
      observations,
      limitations: GATE2_REPAIR_A_COHORT_LIMITATIONS,
      assessment: "deterministic_repair_cohort_a_passed_gate2_incomplete",
    };
...
await main().catch((error) => {
  process.stderr.write(
    `Gate 2 repair cohort A failed: ${error instanceof Error ? error.message : String(error)}\n`,
  );
  process.exitCode = 1;
});
