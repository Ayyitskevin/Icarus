...
export const GATE2_REPAIR_A_PROVIDER = deepFreeze({
  kind: "ollama",
...
export const GATE2_REPAIR_A_COHORT_LIMITATIONS = Object.freeze([
...
const missingConfigCheck = [
  "import os,subprocess,sys,tempfile",
  'happy=subprocess.run([sys.executable,"-m","src.main"],capture_output=True,text=True)',
  'assert happy.returncode == 0 and happy.stdout == "Welcome, traveler.\\n" and happy.stderr == ""',
  "temporary=tempfile.TemporaryDirectory()",
  "environment=os.environ.copy()",
  'environment["PYTHONPATH"]=os.getcwd()',
  'missing=subprocess.run([sys.executable,"-m","src.main"],cwd=temporary.name,env=environment,capture_output=True,text=True)',
  'assert missing.returncode == 1 and missing.stdout == "" and missing.stderr == "Lantern configuration is unavailable.\\n"',
  "temporary.cleanup()",
].join("; ");
const emptyAudienceCheck = [
  "import subprocess,sys",
  'happy=subprocess.run([sys.executable,"-c","from src.greeting import greeting; assert greeting(\\"traveler\\") == \\"Welcome, traveler.\\""],capture_output=True,text=True)',
  'empty=subprocess.run([sys.executable,"-c","from src.greeting import greeting; greeting(\\"\\")"],capture_output=True,text=True)',
  'assert happy.returncode == 0 and happy.stdout == "" and happy.stderr == ""',
  'assert empty.returncode == 1 and empty.stdout == "" and "audience must not be empty" in empty.stderr',
].join("; ");

export const GATE2_REPAIR_A_ORACLES = deepFreeze([
  {
    caseId: "repair-name-whitespace",
    scenarioEvaluatorId: "repair-name-whitespace-evaluator",
    ...
    approvedFiles: [
      {
        op: "modify",
        path: "checks/test_profile.py",
        sha256: "5291787de057aefe35c4e7ffbde16f55f92ddcb6ca429be058fda47c7a8d94af",
        content:
          'from src.format_name import format_name\nfrom src.profile import display_name\n\n\ncases = [\n    ("  ada   lovelace ", "Ada Lovelace"),\n    ("grace\\t\\t hopper", "Grace Hopper"),\n    ("  alan\\n turing  ", "Alan Turing"),\n]\nfor value, expected in cases:\n    assert format_name(value) == display_name(value) == expected\n',
      },
    ],
  },
  {
    caseId: "repair-lantern-missing-config",
    scenarioEvaluatorId: "repair-lantern-missing-config-evaluator",
    baselineOutcome: "failed",
    inputTokens: 150,
    outputTokens: 80,
    check: {
      id: "lantern-missing-config",
      name: "Bounded missing-config error",
      argv: ["python", "-c", missingConfigCheck],
    },
    approvedFiles: [
      {
        op: "modify",
        path: "src/main.py",
        sha256: "3c1dfd9aa08a2e7b62407933a68f3439cd9a413d46c09283b4bad7c59e071b36",
        content:
          'import json\nfrom pathlib import Path\n\nfrom src.greeting import greeting\n\n\ndef main() -> None:\n    try:\n        audience = json.loads(\n            Path("config/app.json").read_text(encoding="utf-8")\n        )["audience"]\n    except FileNotFoundError:\n        raise SystemExit("Lantern configuration is unavailable.") from None\n    print(greeting(audience))\n\n\nif __name__ == "__main__":\n    main()\n',
      },
    ],
  },
  {
    caseId: "repair-lantern-empty-audience",
    scenarioEvaluatorId: "repair-lantern-empty-audience-evaluator",
    baselineOutcome: "failed",
    inputTokens: 150,
    outputTokens: 80,
    check: {
      id: "lantern-empty-audience",
      name: "Lantern audience validation",
      argv: ["python", "-c", emptyAudienceCheck],
    },
    approvedFiles: [
      {
        op: "modify",
        path: "src/greeting.py",
        sha256: "b4ed4855106872f0e6407057f1ed5486d7039c1b0a79f8f4aec8b04ce80776b8",
        content:
          'def greeting(audience: str) -> str:\n    if not audience.strip():\n        raise ValueError("audience must not be empty")\n    return f"Welcome, {audience}."\n',
      },
    ],
  },
  {
    caseId: "repair-basic-newline",
    scenarioEvaluatorId: "repair-basic-newline-evaluator",
    baselineOutcome: "failed",
    inputTokens: 150,
    outputTokens: 80,
    check: {
  ...
  {
    caseId: "repair-cart-empty-list",
    scenarioEvaluatorId: "repair-cart-empty-list-evaluator",
    baselineOutcome: "failed",
    inputTokens: 150,
    outputTokens: 80,
    check: {
      id: "cart-empty-list",
      name: "Cart empty and populated totals",
      argv: ["python", "checks/verify.py"],
    },
    approvedFiles: [
      {
        op: "modify",
        path: "checks/test_cart.py",
        sha256: "683caaf2b7f35914482936f2afbe3eff5df8f43ae5d4d2adb87f2f67c7db13bf",
        content:
...
function fail(message) {
  throw new Error(`Gate 2 repair cohort A: ${message}`);
}
...
export function computeGate2RepairAEvidenceDigest(observationInput) {
  if (
...
export function validateGate2RepairACohortResult(value, manifestInput, manifestSha256) {
  ...
  const repairCaseIds = new Set(GATE2_REPAIR_A_ORACLES.map((entry) => entry.caseId));
  const repairCases = manifest.cases.filter((entry) => repairCaseIds.has(entry.id));
  literal(
    JSON.stringify(repairCases.map((entry) => entry.id)),
    JSON.stringify(GATE2_REPAIR_A_ORACLES.map((entry) => entry.caseId)),
    "manifest repair cohort A",
  );
  ...
  const expectedEffects = [10, 10, 0, 0, 0, 5, 10, 5, 0, 0, true, 5];
  ...
  for (const [index, observationValue] of observations.entries()) {
    ...
    const oracle = GATE2_REPAIR_A_ORACLES[index];
    if (benchmarkCase === undefined || oracle === undefined) fail("repair cohort A index missing");
    ...
    for (const key of ["kind", "model", "adapterVersion", "transport"]) {
      literal(
        provider[key],
        GATE2_REPAIR_A_PROVIDER[key],
        `observations[${index}].provider.${key}`,
      );
    }
    literal(
      JSON.stringify(provider.instructionDigests),
      JSON.stringify(GATE2_REPAIR_A_PROVIDER.instructionDigests),
    ...
    literal(
      ...
      computeGate2RepairAEvidenceDigest(observation),
  ...
  if (
    ...
    fail("repair cohort A does not satisfy manifest retrieval or plan thresholds");
  }
  literal(
    JSON.stringify(result.limitations),
    JSON.stringify(GATE2_REPAIR_A_COHORT_LIMITATIONS),
    "limitations",
  );
  literal(result.assessment, "deterministic_repair_cohort_a_passed_gate2_incomplete", "assessment");
  return value;
}

export function parseAndValidateGate2RepairACohortResult(source, manifest, manifestSha256) {
  return validateGate2RepairACohortResult(parseStrictGate2Json(source), manifest, manifestSha256);
}
