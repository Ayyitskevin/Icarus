...
import {
  ...
  GATE2_SECURITY_REVIEW_PROVIDER,
  validateGate2SecurityReviewCohortResult,
} from "./gate2-security-review-cohort-contract.mjs";
...
const reportPath = path.join(reportDirectory, "gate2-security-review-cohort-report.json");
...
function assertProviderRequest(request, caseId, task, retrieval) {
  ...
  assertCondition(
    request.method === "POST" &&
      ...
      request.body.model === GATE2_SECURITY_REVIEW_PROVIDER.model &&
      ...
      request.body.messages[0].content.includes("untrusted data") &&
      request.body.messages[0].content.includes("Never propose or claim command execution") &&
      request.body.messages[1]?.role === "user" &&
      typeof request.body.messages[1]?.content === "string" &&
      request.body.options?.num_predict === 1_536,
...
async function evaluateCase({
  ...
  assertCondition(
    JSON.stringify({
      ...
      noFinding: securityReview.noFinding,
    }) === JSON.stringify(oracle.response),
    `case ${benchmarkCase.id} security review changed from its frozen evaluator oracle`,
  );
  assertCondition(
    securityReview.baseCommit === baseCommit &&
      securityReview.taskSha256 === benchmarkCase.task.sha256 &&
      securityReview.retrievalDigestSha256 === retrieval.digestSha256 &&
      securityReview.usage.inputTokens === oracle.inputTokens &&
      securityReview.usage.outputTokens === oracle.outputTokens &&
      securityReview.usage.estimatedCostUsd === 0,
    `case ${benchmarkCase.id} security review provenance or accounting changed`,
  );
  ...
  const observation = {
    ...
    securityReviewDigestSha256: securityReview.digestSha256,
    provider: GATE2_SECURITY_REVIEW_PROVIDER,
    selectedContext,
    ...
    usage: {
      inputTokens: securityReview.usage.inputTokens,
      outputTokens: securityReview.usage.outputTokens,
      estimatedCostUsd: securityReview.usage.estimatedCostUsd,
      actualBilledUsd: null,
      latencyMs: securityReview.usage.latencyMs,
    },
...
async function prepareReportDestination() {
  ...
  if (existing !== null) {
    assertCondition(
      ...
      "existing Gate 2 security-review cohort report is unsafe",
    );
...
async function persistReport(report, manifest, manifestSha256) {
  ...
  validateGate2SecurityReviewCohortResult(report, manifest, manifestSha256);
  const temporaryPath = path.join(
    reportDirectory,
    `.gate2-security-review-cohort-${process.pid}-${randomUUID()}`,
  );
  ...
  assertCondition(
    ...
    "persisted Gate 2 security-review cohort report changed bytes",
  );
  validateGate2SecurityReviewCohortResult(
...
async function main() {
  assertCondition(
    ...
    "usage: node scripts/gate2-security-review-cohort.mjs",
  );
  ...
  const securityReviewCases = loaded.manifest.cases.filter(
    (entry) => entry.class === "security_review",
  );
  assertCondition(
    JSON.stringify(securityReviewCases.map((entry) => entry.id)) ===
      JSON.stringify(GATE2_SECURITY_REVIEW_ORACLES.map((entry) => entry.caseId)),
    "manifest security-review cohort changed from the evaluator registry",
  );
  const repositories = new Map(
    loaded.manifest.repositories.map((repository) => [repository.id, repository]),
  );
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "icarus-gate2-security-review-"));
  const providerServer = await startLoopbackProvider(GATE2_SECURITY_REVIEW_ORACLES);
  try {
    const providerConfig = createProviderConfig({
      kind: "ollama",
      model: GATE2_SECURITY_REVIEW_PROVIDER.model,
    ...
    const observations = [];
    for (const [index, benchmarkCase] of securityReviewCases.entries()) {
    ...
    assertCondition(
      providerServer.requests.length === securityReviewCases.length &&
        providerServer.remaining() === 0,
      "deterministic provider did not execute exactly the cohort request count",
    );
    const macroRecall =
    ...
    const digestProvenanceCoverage =
      observations.reduce(
        (sum, entry) => sum + entry.retrievalMetrics.digestProvenanceCoverage,
        0,
      ) / observations.length;
    const report = {
      schemaVersion: 1,
      ...
      counts: {
        ...
        cohortCases: securityReviewCases.length,
      ...
      retrievalAggregate: { macroRecall, macroPrecision, digestProvenanceCoverage },
      effects: {
...
await main().catch((error) => {
  process.stderr.write(
    `Gate 2 security-review cohort failed: ${error instanceof Error ? error.message : String(error)}\n`,
  );
  process.exitCode = 1;
});
