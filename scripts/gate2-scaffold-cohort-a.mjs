...
} from "./gate2-scaffold-cohort-a-contract.mjs";
...
const reportPath = path.join(reportDirectory, "gate2-scaffold-cohort-a-report.json");
const PYTHON_IMAGE =
...
function planResponse(benchmarkCase, repository, oracle) {
  ...
  return {
    summary: "Apply the bounded Gate 2 scaffold A fixture.",
    steps: ["Apply only the approved scaffold", "Run the registered scenario check"],
...
async function patchResponse(benchmarkCase, repository, oracle) {
  ...
  return {
    summary: "Apply only the pinned Gate 2 scaffold A.",
    edits: oracle.approvedFiles.map((file) => {
      if (file.op === "create") {
        return {
          ...
          rationale: "Create only the manifest-approved scaffold module.",
        };
      }
      ...
      return {
        ...
        content: null,
        rationale: "Apply only the manifest-approved scaffold bytes.",
      };
    }),
  };
}

async function providerQueue(scaffoldCases, repositories) {
  const queue = [];
  for (const [index, benchmarkCase] of scaffoldCases.entries()) {
  ...
  for (const [index, request] of requests.entries()) {
    ...
    assertCondition(
      request.caseId === benchmarkCase.id &&
        ...
        body.model === GATE2_SCAFFOLD_A_PROVIDER.model &&
        body.stream === false &&
        body.think === false &&
...
async function evaluateCase({
  ...
  const providerConfig = createProviderConfig({
    kind: "ollama",
    model: GATE2_SCAFFOLD_A_PROVIDER.model,
  ...
  const reviewed = await runtime.service.approvePlan(
    planned.id,
    planned.planSha256,
    "gate2-scaffold-a-fixture-operator",
  );
  assertCondition(
...
async function prepareReportDestination() {
  ...
  if (existing !== null) {
    assertCondition(
      ...
      "existing Gate 2 scaffold A report is unsafe",
    );
...
async function persistReport(report, manifest, manifestSha256) {
  ...
  validateGate2ScaffoldACohortResult(report, manifest, manifestSha256);
  const temporaryPath = path.join(
    reportDirectory,
    `.gate2-scaffold-cohort-a-${process.pid}-${randomUUID()}`,
  );
  ...
  assertCondition(
    ...
    "persisted Gate 2 scaffold A report changed bytes",
  );
  parseAndValidateGate2ScaffoldACohortResult(persisted.toString("utf8"), manifest, manifestSha256);
}

async function main() {
  assertCondition(process.argv.length === 2, "usage: node scripts/gate2-scaffold-cohort-a.mjs");
  ...
  const allScaffoldCases = loaded.manifest.cases.filter((entry) => entry.class === "scaffold");
  assertCondition(
    JSON.stringify(allScaffoldCases.map((entry) => entry.id)) ===
      JSON.stringify(GATE2_SCAFFOLD_A_MANIFEST_CASE_IDS),
    "manifest scaffold class changed from the evaluator registry",
  );
  ...
  assertCondition(
    ...
    "protected scaffold exclusion changed",
  );
  ...
  assertCondition(
    ...
    "manifest scaffold A cohort changed from the evaluator registry",
  );
  const repositories = new Map(
    loaded.manifest.repositories.map((repository) => [repository.id, repository]),
  );
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "icarus-gate2-scaffold-a-"));
  const providerServer = await startLoopbackProvider(
    await providerQueue(scaffoldCases, repositories),
  );
  ...
  globalThis.fetch = async (input, init) => {
    ...
    if (
      ...
      throw new Error(`Gate 2 scaffold A blocked non-loopback fetch: ${target.origin}`);
    }
  ...
  try {
    ...
    assertCondition(
      providerServer.requests.length === 8 &&
        providerServer.remaining() === 0 &&
        fetchEffects.loopbackProviderRequests === 8 &&
...
await main().catch((error) => {
  process.stderr.write(
    `Gate 2 scaffold A cohort failed: ${error instanceof Error ? error.message : String(error)}\n`,
  );
  process.exitCode = 1;
});
