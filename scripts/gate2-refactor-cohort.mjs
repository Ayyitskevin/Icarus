...
} from "./gate2-refactor-cohort-contract.mjs";
...
const reportPath = path.join(reportDirectory, "gate2-refactor-cohort-report.json");
const PYTHON_IMAGE =
...
function planResponse(benchmarkCase, repository, oracle) {
  ...
  return {
    summary: "Apply the bounded Gate 2 refactor fixture.",
    steps: ["Apply only the approved refactor", "Run the registered scenario check"],
...
async function patchResponse(benchmarkCase, repository, oracle) {
  ...
  return {
    summary: "Apply only the pinned Gate 2 refactor.",
    edits: oracle.approvedFiles.map((file) => {
      if (file.op === "create") {
        return {
          ...
          rationale: "Create only the manifest-approved refactor module.",
        };
      }
      ...
      return {
        ...
        content: null,
        rationale: "Apply only the manifest-approved refactor bytes.",
      };
    }),
  };
}

async function providerQueue(refactorCases, repositories) {
  const queue = [];
  for (const [index, benchmarkCase] of refactorCases.entries()) {
  ...
  for (const [index, request] of requests.entries()) {
    ...
    assertCondition(
      request.caseId === benchmarkCase.id &&
        ...
        body.model === GATE2_REFACTOR_PROVIDER.model &&
        body.stream === false &&
        body.think === false &&
...
async function evaluateCase({
  ...
  const providerConfig = createProviderConfig({
    kind: "ollama",
    model: GATE2_REFACTOR_PROVIDER.model,
  ...
  const reviewed = await runtime.service.approvePlan(
    planned.id,
    planned.planSha256,
    "gate2-refactor-fixture-operator",
  );
  assertCondition(
...
async function prepareReportDestination() {
  ...
  if (existing !== null) {
    assertCondition(
      ...
      "existing Gate 2 refactor report is unsafe",
    );
...
async function persistReport(report, manifest, manifestSha256) {
  ...
  validateGate2RefactorCohortResult(report, manifest, manifestSha256);
  const temporaryPath = path.join(
    reportDirectory,
    `.gate2-refactor-cohort-${process.pid}-${randomUUID()}`,
  );
  ...
  assertCondition(
    ...
    "persisted Gate 2 refactor report changed bytes",
  );
  parseAndValidateGate2RefactorCohortResult(persisted.toString("utf8"), manifest, manifestSha256);
}

async function main() {
  assertCondition(process.argv.length === 2, "usage: node scripts/gate2-refactor-cohort.mjs");
  ...
  const refactorCases = loaded.manifest.cases.filter((entry) => entry.class === "refactor");
  assertCondition(
    JSON.stringify(refactorCases.map((entry) => entry.id)) ===
      JSON.stringify(GATE2_REFACTOR_ORACLES.map((entry) => entry.caseId)),
    "manifest refactor cohort changed from the evaluator registry",
  );
  const repositories = new Map(
    loaded.manifest.repositories.map((repository) => [repository.id, repository]),
  );
  const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "icarus-gate2-refactor-"));
  const providerServer = await startLoopbackProvider(
    await providerQueue(refactorCases, repositories),
  );
  ...
  globalThis.fetch = async (input, init) => {
    ...
    if (
      ...
      throw new Error(`Gate 2 refactor blocked non-loopback fetch: ${target.origin}`);
    }
...
await main().catch((error) => {
  process.stderr.write(
    `Gate 2 refactor cohort failed: ${error instanceof Error ? error.message : String(error)}\n`,
  );
  process.exitCode = 1;
});
