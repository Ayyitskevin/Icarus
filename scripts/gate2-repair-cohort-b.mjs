...
} from "./gate2-repair-cohort-b-contract.mjs";
...
const reportPath = path.join(reportDirectory, "gate2-repair-cohort-b-report.json");
const PYTHON_IMAGE =
...
function operatorTargets(benchmarkCase, repository) {
  ...
  assertCondition(
    ...
    `${benchmarkCase.id} repair cohort B requires existing targets only`,
  );
  ...
  for (const [index, request] of requests.entries()) {
    ...
    assertCondition(
      request.caseId === benchmarkCase.id &&
        ...
        body.model === GATE2_REPAIR_B_PROVIDER.model &&
        body.stream === false &&
        body.think === false &&
...
async function evaluateCase({
  ...
  const providerConfig = createProviderConfig({
    kind: "ollama",
    model: GATE2_REPAIR_B_PROVIDER.model,
...
async function persistReport(report, manifest, manifestSha256) {
  ...
  validateGate2RepairBCohortResult(report, manifest, manifestSha256);
  const temporaryPath = path.join(
    reportDirectory,
    `.gate2-repair-cohort-b-${process.pid}-${randomUUID()}`,
  );
  ...
  parseAndValidateGate2RepairBCohortResult(persisted.toString("utf8"), manifest, manifestSha256);
}

async function main() {
  assertCondition(process.argv.length === 2, "usage: node scripts/gate2-repair-cohort-b.mjs");
  ...
  assertCondition(
    ...
    "manifest repair cohort B changed from the evaluator registry",
  );
  ...
  try {
    ...
    assertCondition(
      ...
      "repair cohort B sandbox check evidence changed",
    );
    const report = {
      schemaVersion: 1,
...
await main().catch((error) => {
  process.stderr.write(
    `Gate 2 repair cohort B failed: ${error instanceof Error ? error.message : String(error)}\n`,
  );
  process.exitCode = 1;
});
