...
approval in the ordinary approvals pipeline and one
`headless.worker.apply_requested` intent, re-drives from the boundary, and
settles `icarus.headless.worker-application.v1`. A crashed application epoch
