...
| 2 | Context and agent quality — **one-case retrieval baseline measured** | initial pinned unfamiliar-codebase retrieval is recall `1.0` / precision `1.0` against one eligible negative example; exit still requires measured explanation/security/refactor evals, a versioned 30-task benchmark with retrieval recall ≥0.90 and precision ≥0.60, and first-pass plan acceptance ≥80% |
...
The full explanation class remains honestly unsupported until an actual
explanation run produces validated
