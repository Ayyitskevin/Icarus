...
ADR 0044 establishes a headless-first workstream for Mickey-like AI workspaces. Icarus remains the authority kernel. H0 is the versioned `icarus.headless.history.v1` JSONL trajectory export; H1 adds digestable default-deny profiles; H2 adds a bounded one-task worker; H3 adds replay/fork/resume; H4 adds isolated child runs; H5 may add pinned out-of-process adapters such as DeepSeek Harness.
