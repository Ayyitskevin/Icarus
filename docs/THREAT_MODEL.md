...
| A caller or hostile GitHub response widens fake landing authority, rotates credentials mid-operation, replays a mutation, or converts uncertainty into success | accept only an atomically claimed admitted request ID; revalidate its landing/profile/manifest/text digests before credential resolution; derive every route/header/body internally; pin one actor-proved credential to one operation; require an exact same-operation base proof before head absence; synchronously reject duplicate request IDs; bound and strictly parse response bytes; normalize provider owner/repository identities while keeping refs case-sensitive; mark possibly delivered malformed/5xx mutations ambiguous; expose no live transport or automatic retry | deterministic fake-transport tests cover fixed argument boundaries, immutable body reconstruction, secret-free digests/results, operation credential pin/drop, exact actor/base/head evidence, mixed-case identities, whole-list refusal, response size/UTF-8/JSON/header failures, 403/429 classification, 409/422 conflicts, timeout phase, ambiguous post-read continuation, and sequential/concurrent duplicate delivery; remote ledger/restart settlement and real-provider evidence remain open |
