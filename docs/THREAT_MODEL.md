...
| A caller pins a landing's commit timestamp to backdate a commit, or pinning it silently backdates the run's whole durable evidence trail | `PrepareLandingInput.commitEpochSeconds` is optional and, when supplied, applies to the candidate commit instant ALONE (ADR 0051). The coordinator's `now` seam — which would have needed no new input — is deliberately not used for this, because it is the clock for every admission, settlement, event and state transition the coordinator writes, and backdating those would make the evidence trail misreport when things happened while everything still passed. A pinned epoch must be a non-negative safe integer, must not exceed the observed clock, and remains subject to `commitEpochToGitInstant`'s bound. It is stored durably on the landing record exactly as an observed value is, so it is auditable against the run's own event timestamps rather than hidden. The authority that a particular epoch may be pinned belongs to the caller, not the coordinator; for Gate 1 a wrong pin does not produce misleading evidence, because the manifest's `expectedCandidateCommitSha1` is derived from the epoch and a mismatch fails the case | a pinned landing and an unpinned landing built from the same fixture agree on every other durable timestamp and differ only in the commit instant; a pin later than the observed clock is refused with `INVALID_LANDING_TIMESTAMP` and no durable landing is created; negative, fractional and `NaN` pins are refused the same way; with no pin supplied the observed clock is used, unchanged |
| A Gate 1 live-evidence profile that is correct about repositories redirects the run at a different model or spends real money the reviewed contract never authorized, or a caller presents an edited manifest under the reviewed manifest's digest so that every binding below is evaluated against work nobody approved | `assertLiveEvidenceProfileMatchesManifest` takes the manifest as `Uint8Array` BYTES and nothing else, computes `sha256(manifestBytes)` itself, and parses the manifest from those same bytes, so what is validated is necessarily what was hashed (ADR 0052). It previously took the parsed manifest and its digest as two independent parameters and compared the profile's pin against the digest STRING, which compared one caller claim to another: an edited manifest supplied with the reviewed digest was admitted and every check below ran against the edited one, making ADR 0045 property 1 and ADR 0050's pins unreachable. With one parameter the mismatch is unrepresentable rather than merely detected. A non-`Uint8Array` is refused at runtime rather than trusted from the annotation, invalid UTF-8 is refused rather than replaced (a replacement character would validate text other than the hashed bytes), and bytes that hash correctly but are not a strict-JSON object are refused after the digest check. On top of that it binds the manifest fields that decide model and money, not only those that decide repositories: `profile.provider.kind` must equal the case's `modelAdapter.provider`; a case declaring `modelAdapter.paid: false` refuses any profile token rate other than `null` or `0`; and `profile.budgets.maxSpendUsd` must not exceed the case's `budgets.maxCostUsd`, which the committed manifest pins at 0. The `modelAdapter` block is strict-decoded against its exact nine keys, and a case missing `modelAdapter` or `budgets.maxCostUsd` is refused rather than skipped, so absence cannot read as permission. `model`, `adapterVersion`, `transport`, `expectedRequests` and `credentials` are deliberately unbound and recorded as such in ADR 0050, because they describe the offline replay rather than what a live run may do. The refusal lands at authorization, before any effect — previously the only barrier was `store.ts`'s project-ceiling check at admission of the first provider call, which is executor policy rather than record authority | a profile carrying the exact reviewed manifest digest, the exact case ids, correct repository identities and a self-consistent digest-bound approval, but pinning `anthropic/claude-opus-5` at `maxSpendUsd: 500`, is refused while `assertLiveEvidenceProfileApproved` still passes on it — proving the manifest binding is what catches it; a same-kind provider carrying a non-zero token rate against an unpaid case is refused; `maxSpendUsd: 0.01` against `maxCostUsd: 0` is refused; null rates are accepted because a loopback provider charges nothing; manifest cases missing `modelAdapter` or `budgets.maxCostUsd` are refused; the committed three-case manifest still authorizes a conforming profile; `scripts/security-check.mjs`'s `liveEvidenceAuthorityIsClosedBoundAndInert` pins the closed effect set, the four binding messages, the runtime freeze and chain-order refusals, and the absence of any I/O, fetch or `api.github.com` reference in either module — proven able to fail by injecting a forbidden effect literal and a removed binding |
| A caller widens Gate 1 live-evidence authority after the human approval that granted it, or reports a landing chain it never performed | the authorization `authorizeLiveEvidenceRun` returns is frozen at every level and copied before freezing, so `effects.push(...)` and a raised `maxSpendUsd` throw instead of widening a digest-bound approval; `LiveEvidenceEffectLedger` re-checks and copies the authorization rather than retaining it, refusing at construction any effect list that is not the closed set in order, any empty or repeated case-id list, and any `NaN`/`Infinity`/non-positive ceiling, which would otherwise make every `next > ceiling` comparison false; the pinned provider's credential (`OPENAI_API_KEY`/`ANTHROPIC_API_KEY`, resolved through the same table `createGateway` reads) is preflighted alongside each landing credential before any remote effect, and the predicate is the strictest one any consumer applies — a string of 8 to 512 characters with no whitespace, NUL, or other control character, which is `GithubGateway`'s rule and a strict subset of the model gateways' — because a preflight asserting a weaker predicate than its consumer admits a run it cannot deliver; the value reaches one predicate returning a boolean and never a message, the authorization, the ledger, or durable state; manifest case ids and `owner/repository` identities must be distinct, so one landing cannot receive two cases' draft-pull-request POSTs; and each case must walk the authorized chain — upload, absent-only ref, draft PR, receipt — forwards with no skipped or repeated-backwards stage, because counting alone cannot prove a landing happened | an authorization is frozen at every level and mutation attempts throw `TypeError`; a hand-built authorization naming `github.ref.force_update`, reordering the closed set, repeating a case id, or carrying an unbounded ceiling is refused at ledger construction; a ledger built from a mutable authorization still refuses the effect and the raised ceiling appended afterwards; OpenAI and Anthropic profiles with the landing token present but no model key refuse before any effect, and the required name is asserted through `providerCredentialEnvironmentName` rather than a literal; a manifest mapping two cases to one repository, or repeating a case id, is refused while the reviewed three-repository manifest still passes; a receipt before upload, a skipped ref-creation stage, and an upload after ref creation are each refused, while repeated uploads within the current stage are admitted; no credential value appears in any authorization, refusal, or ledger summary |
...
As of S2b-ii-c the coordinator drives the read-only preflight plus all three
mutations: immutable object upload, the absent-only reference creation, and
the one-POST-ever draft pull-request creation with its reconciliation list
read and immutable receipt. ADR 0043's two open contract questions remain
open and are still not relied upon for safety: a case-sensitive head filter
or a truncated page fails closed into a 422 refusal or an ambiguity hold —
never a duplicate pull request — and the live observation that settles them
belongs to the S3 live-evidence profile. Coordinator-side
