...
| [0029](adr/0029-browser-approval-authority.md) | Fresh-origin browser mutation session and fenced Linux run actions | Accepted — ADR 0040 technical evidence is complete; origin portability remains held pending ADR 0040 human residual-risk acceptance |
...
| [0040](adr/0040-chromium-resolved-localhost-origins.md) | Chromium-resolved 128-bit `.localhost` mutation origins over exact `127.0.0.1` | Candidate — implementation and exact-head macOS/Windows real-Chrome evidence complete at `eb01b64` (run `30618043377`); explicit human acceptance of the operator browser/resolver/proxy risk pending |
