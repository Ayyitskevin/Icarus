...
import {
  computeGate2RefactorEvidenceDigest,
  GATE2_REFACTOR_COHORT_LIMITATIONS,
  GATE2_REFACTOR_ORACLES,
  GATE2_REFACTOR_PROVIDER,
  parseAndValidateGate2RefactorCohortResult,
  validateGate2RefactorCohortResult,
} from "../../scripts/gate2-refactor-cohort-contract.mjs";
...
function result(): Record<string, any> {
  ...
  const observations = GATE2_REFACTOR_ORACLES.map((oracle, index) => {
    ...
    if (firstExpectedTarget === undefined) throw new Error("refactor case has no expected target");
    if (!sourcePaths.has(firstExpectedTarget)) {
      ...
      if (anchor === undefined) throw new Error("refactor fixture has no existing anchor");
    ...
    const observation: Record<string, any> = {
      ...
      provider: {
        ...GATE2_REFACTOR_PROVIDER,
        loopbackRequests: 2,
        instructionDigests: [...GATE2_REFACTOR_PROVIDER.instructionDigests],
    ...
    observation.scenarioEvidenceSha256 = computeGate2RefactorEvidenceDigest(observation);
    return observation;
  });
  return {
    schemaVersion: 1,
    ...
    effects: {
      providerCalls: 10,
      loopbackProviderRequests: 10,
      externalNetworkRequests: 0,
      remoteMutations: 0,
      sourceCheckoutMutations: 0,
      privateWorkspaceMutations: 5,
      sandboxCheckExecutions: 10,
      icarusRegisteredCheckExecutions: 5,
    ...
    observations,
    limitations: [...GATE2_REFACTOR_COHORT_LIMITATIONS],
    assessment: "deterministic_refactor_cohort_passed_gate2_incomplete",
  };
}
...
describe("Gate 2 refactor cohort result contract", () => {
  it("accepts exactly the five manifest-bound production-lifecycle observations", () => {
    const candidate = result();
    expect(validateGate2RefactorCohortResult(candidate, manifest, manifestSha256)).toBe(candidate);
    expect(candidate.observations.map((entry: Record<string, any>) => entry.caseId)).toEqual(
      GATE2_REFACTOR_ORACLES.map((entry) => entry.caseId),
    );
  });
  ...
  it("rejects widened effects, false completion, forged counts, shapes, and limitations", () => {
    ...
    for (const attack of attacks) {
      expect(() => validateGate2RefactorCohortResult(attack, manifest, manifestSha256)).toThrow();
    }
  });
  ...
  it("rejects changed authority, plans, checks, final bytes, durability, and evidence", () => {
    ...
    requiredAt(attacks, 2).observations[0].baseline.outcome = "failed";
  ...
  it("accepts extra eligible context only with honestly recomputed retrieval metrics", () => {
    ...
    if (benchmarkCase === undefined) throw new Error("refactor fixture case missing");
    ...
    observation.scenarioEvidenceSha256 = computeGate2RefactorEvidenceDigest(observation);
    expect(() => validateGate2RefactorCohortResult(candidate, manifest, manifestSha256)).toThrow();
  });

  it("rejects a manifest that rebinds a refactor case to another evaluator", () => {
    const rebound = structuredClone(manifest);
    const benchmarkCase = rebound.cases.find(
      (entry) => entry.id === GATE2_REFACTOR_ORACLES[0]?.caseId,
    );
    if (benchmarkCase === undefined) throw new Error("refactor fixture case missing");
    benchmarkCase.expectedOutcome.scenarioEvaluatorId = "replacement-evaluator";
    expect(() => validateGate2RefactorCohortResult(result(), rebound, manifestSha256)).toThrow();
  });
  ...
  it("strict parsing rejects duplicate members, oversized input, and excessive depth", () => {
    ...
    for (const attack of [duplicate, oversized, tooDeep]) {
      expect(() =>
        parseAndValidateGate2RefactorCohortResult(attack, manifest, manifestSha256),
      ).toThrow();
    }
  });
});
