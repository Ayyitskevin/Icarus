...
import {
  computeGate2ScaffoldAEvidenceDigest,
  GATE2_SCAFFOLD_A_COHORT_LIMITATIONS,
  GATE2_SCAFFOLD_A_EXCLUDED_CASE_ID,
  GATE2_SCAFFOLD_A_MANIFEST_CASE_IDS,
  GATE2_SCAFFOLD_A_ORACLES,
  GATE2_SCAFFOLD_A_PROVIDER,
  parseAndValidateGate2ScaffoldACohortResult,
  validateGate2ScaffoldACohortResult,
} from "../../scripts/gate2-scaffold-cohort-a-contract.mjs";
...
function result(): Record<string, any> {
  ...
  const observations = GATE2_SCAFFOLD_A_ORACLES.map((oracle, index) => {
    ...
    const sourcePaths = new Set(repository.files.map((file: Record<string, string>) => file.path));
    const operatorSelectedTargets = [...expectedChangedPaths];
    const firstExpectedTarget = operatorSelectedTargets[0];
    if (firstExpectedTarget === undefined) throw new Error("scaffold case has no expected target");
    if (!sourcePaths.has(firstExpectedTarget)) {
      const anchor = repository.files
        .map((file: Record<string, string>) => file.path)
        .filter((entry: string) => entry.localeCompare(firstExpectedTarget) < 0)
        .sort((left: string, right: string) => left.localeCompare(right))[0];
      if (anchor === undefined) throw new Error("scaffold fixture has no existing anchor");
      operatorSelectedTargets.push(anchor);
    }
    operatorSelectedTargets.sort((left, right) => left.localeCompare(right));
    const observation: Record<string, any> = {
      ...
      provider: {
        ...GATE2_SCAFFOLD_A_PROVIDER,
        loopbackRequests: 2,
        instructionDigests: [...GATE2_SCAFFOLD_A_PROVIDER.instructionDigests],
    ...
    observation.scenarioEvidenceSha256 = computeGate2ScaffoldAEvidenceDigest(observation);
    return observation;
  });
  return {
    schemaVersion: 1,
    ...
    observations,
    limitations: [...GATE2_SCAFFOLD_A_COHORT_LIMITATIONS],
    assessment: "deterministic_scaffold_a_cohort_passed_gate2_incomplete",
  };
}
...
describe("Gate 2 scaffold A cohort result contract", () => {
  it("accepts exactly the four policy-compatible production-lifecycle observations", () => {
    const candidate = result();
    expect(validateGate2ScaffoldACohortResult(candidate, manifest, manifestSha256)).toBe(candidate);
    expect(candidate.observations.map((entry: Record<string, any>) => entry.caseId)).toEqual(
      GATE2_SCAFFOLD_A_ORACLES.map((entry) => entry.caseId),
    );
  });

  it("binds the complete scaffold class while excluding the protected migration case", () => {
    expect(
      manifest.cases.filter((entry) => entry.class === "scaffold").map((entry) => entry.id),
    ).toEqual(GATE2_SCAFFOLD_A_MANIFEST_CASE_IDS);
    expect(GATE2_SCAFFOLD_A_ORACLES.map((entry) => entry.caseId)).not.toContain(
      GATE2_SCAFFOLD_A_EXCLUDED_CASE_ID,
    );
    const rebound = structuredClone(manifest);
    const excluded = rebound.cases.find((entry) => entry.id === GATE2_SCAFFOLD_A_EXCLUDED_CASE_ID);
    if (excluded === undefined) throw new Error("protected scaffold case missing");
    excluded.expectedOutcome.expectedChangedPaths = ["schema/current.sql"];
    expect(() => validateGate2ScaffoldACohortResult(result(), rebound, manifestSha256)).toThrow();
  });
  ...
  it("rejects widened effects, false completion, forged counts, shapes, and limitations", () => {
    ...
    for (const attack of attacks) {
      expect(() => validateGate2ScaffoldACohortResult(attack, manifest, manifestSha256)).toThrow();
    }
  });
  ...
  it("accepts extra eligible context only with honestly recomputed retrieval metrics", () => {
    ...
    if (benchmarkCase === undefined) throw new Error("scaffold fixture case missing");
    ...
    observation.scenarioEvidenceSha256 = computeGate2ScaffoldAEvidenceDigest(observation);
    expect(() => validateGate2ScaffoldACohortResult(candidate, manifest, manifestSha256)).toThrow();
  });

  it("accepts an honest greeting-context miss only while macro recall stays above threshold", () => {
    const candidate = result();
    const observation = candidate.observations.find(
      (entry: Record<string, any>) => entry.caseId === "scaffold-greeting-command",
    ) as Record<string, any> | undefined;
    if (observation === undefined) throw new Error("greeting scaffold observation missing");
    observation.selectedContext = observation.selectedContext.filter(
      (entry: Record<string, string>) => entry.path !== "README.md",
    );
    observation.retrievalMetrics.recall = 0.75;
    observation.retrievalMetrics.precision = 1;
    observation.scenarioEvidenceSha256 = computeGate2ScaffoldAEvidenceDigest(observation);
    candidate.retrievalAggregate.macroRecall = 0.9375;
    candidate.retrievalAggregate.macroPrecision = 1;
    expect(validateGate2ScaffoldACohortResult(candidate, manifest, manifestSha256)).toBe(candidate);

    observation.retrievalMetrics.recall = 1;
    observation.scenarioEvidenceSha256 = computeGate2ScaffoldAEvidenceDigest(observation);
    expect(() => validateGate2ScaffoldACohortResult(candidate, manifest, manifestSha256)).toThrow();
  });

  it("rejects a manifest that rebinds a scaffold case to another evaluator", () => {
    const rebound = structuredClone(manifest);
    const benchmarkCase = rebound.cases.find(
      (entry) => entry.id === GATE2_SCAFFOLD_A_ORACLES[0]?.caseId,
    );
    if (benchmarkCase === undefined) throw new Error("scaffold fixture case missing");
  ...
  it("strict parsing rejects duplicate members, oversized input, and excessive depth", () => {
    ...
    for (const attack of [duplicate, oversized, tooDeep]) {
      expect(() =>
        parseAndValidateGate2ScaffoldACohortResult(attack, manifest, manifestSha256),
      ).toThrow();
    }
  });
});
