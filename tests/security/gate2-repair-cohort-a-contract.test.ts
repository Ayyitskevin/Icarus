...
} from "../../scripts/gate2-repair-cohort-a-contract.mjs";
...
function result(): Record<string, any> {
  ...
  const observations = GATE2_REPAIR_A_ORACLES.map((oracle, index) => {
    ...
    const observation: Record<string, any> = {
      ...
      provider: {
        ...GATE2_REPAIR_A_PROVIDER,
        loopbackRequests: 2,
        instructionDigests: [...GATE2_REPAIR_A_PROVIDER.instructionDigests],
...
describe("Gate 2 repair cohort A result contract", () => {
  it("accepts exactly the five manifest-bound production-lifecycle observations", () => {
    const candidate = result();
    expect(validateGate2RepairACohortResult(candidate, manifest, manifestSha256)).toBe(candidate);
    expect(candidate.observations.map((entry: Record<string, any>) => entry.caseId)).toEqual(
      GATE2_REPAIR_A_ORACLES.map((entry) => entry.caseId),
    );
  });
  ...
  it("rejects widened effects, false completion, forged counts, shapes, and limitations", () => {
    ...
    for (const attack of attacks) {
      expect(() => validateGate2RepairACohortResult(attack, manifest, manifestSha256)).toThrow();
    }
  });
  ...
  it("accepts extra eligible context only with honestly recomputed retrieval metrics", () => {
    ...
    observation.scenarioEvidenceSha256 = computeGate2RepairAEvidenceDigest(observation);
    expect(() => validateGate2RepairACohortResult(candidate, manifest, manifestSha256)).toThrow();
  });
  ...
  it("rejects a manifest that rebinds a repair case to another evaluator", () => {
    ...
    const benchmarkCase = rebound.cases.find(
      (entry) => entry.id === GATE2_REPAIR_A_ORACLES[0]?.caseId,
    );
    if (benchmarkCase === undefined) throw new Error("repair fixture case missing");
    benchmarkCase.expectedOutcome.scenarioEvaluatorId = "replacement-evaluator";
    expect(() => validateGate2RepairACohortResult(result(), rebound, manifestSha256)).toThrow();
  });
  ...
  it("strict parsing rejects duplicate members, oversized input, and excessive depth", () => {
    ...
    for (const attack of [duplicate, oversized, tooDeep]) {
      expect(() =>
        parseAndValidateGate2RepairACohortResult(attack, manifest, manifestSha256),
      ).toThrow();
    }
  });
});
