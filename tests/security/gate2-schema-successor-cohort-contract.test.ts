...
} from "../../scripts/gate2-schema-successor-cohort-contract.mjs";

const repositoryRoot = new URL("../../", import.meta.url);
const manifestUrl = new URL("../../fixtures/evals/gate2/manifest.v2.json", import.meta.url);
...
function result(): Record<string, any> {
  ...
  const observations = GATE2_SCHEMA_SUCCESSOR_ORACLES.map((oracle, index) => {
    ...
    if (firstExpectedTarget === undefined) throw new Error("successor case has no expected target");
    if (!sourcePaths.has(firstExpectedTarget)) {
      ...
      if (anchor === undefined) throw new Error("successor fixture has no existing anchor");
    ...
    const observation: Record<string, any> = {
      ...
      provider: {
        ...GATE2_SCHEMA_SUCCESSOR_PROVIDER,
        loopbackRequests: 2,
        instructionDigests: [...GATE2_SCHEMA_SUCCESSOR_PROVIDER.instructionDigests],
  ...
  return {
    ...
    effects: {
      providerCalls: 4,
      loopbackProviderRequests: 4,
      externalNetworkRequests: 0,
      remoteMutations: 0,
      sourceCheckoutMutations: 0,
      privateWorkspaceMutations: 2,
      sandboxCheckExecutions: 4,
      icarusRegisteredCheckExecutions: 2,
      liveDatabaseConnections: 0,
      offlineInMemoryDatabaseChecks: 4,
...
describe("Gate 2 schema successor cohort result contract", () => {
  it("accepts exactly the two v2 schema-successor lifecycle observations", () => {
    const candidate = result();
    expect(validateGate2SchemaSuccessorCohortResult(candidate, manifest, manifestSha256)).toBe(
      candidate,
    );
    expect(candidate.observations.map((entry: Record<string, any>) => entry.caseId)).toEqual(
      GATE2_SCHEMA_SUCCESSOR_ORACLES.map((entry) => entry.caseId),
    );
  });

  it("binds v2 lineage and the two host-compatible replacement identities", () => {
    expect(manifest.schemaVersion).toBe(2);
    expect(manifest.supersedes?.benchmarkRevision).toBe("gate2-thirty-task-v1");
    expect(GATE2_SCHEMA_SUCCESSOR_ORACLES.map((entry) => entry.caseId)).toEqual(
      GATE2_SCHEMA_SUCCESSOR_MANIFEST_CASE_IDS,
    );
    for (const caseId of GATE2_SCHEMA_SUCCESSOR_MANIFEST_CASE_IDS) {
      const benchmarkCase = manifest.cases.find((entry) => entry.id === caseId);
      expect(
        benchmarkCase?.expectedOutcome.expectedChangedPaths.some((entry) =>
          entry.split("/").includes("migrations"),
        ),
      ).toBe(false);
    }
  });
  ...
  it("rejects widened effects, false completion, forged counts, shapes, and limitations", () => {
    ...
    for (const attack of attacks) {
      expect(() =>
        validateGate2SchemaSuccessorCohortResult(attack, manifest, manifestSha256),
      ).toThrow();
    }
  });
  ...
  it("accepts extra eligible context only with honestly recomputed retrieval metrics", () => {
    ...
    if (benchmarkCase === undefined) throw new Error("successor fixture case missing");
    ...
    observation.scenarioEvidenceSha256 = computeGate2SchemaSuccessorEvidenceDigest(observation);
    expect(() =>
      validateGate2SchemaSuccessorCohortResult(candidate, manifest, manifestSha256),
    ).toThrow();
  });

  it("rejects a manifest that rebinds a successor case to another evaluator", () => {
    const rebound = structuredClone(manifest);
    const benchmarkCase = rebound.cases.find(
      (entry) => entry.id === GATE2_SCHEMA_SUCCESSOR_ORACLES[0]?.caseId,
    );
    if (benchmarkCase === undefined) throw new Error("successor fixture case missing");
    benchmarkCase.expectedOutcome.scenarioEvaluatorId = "replacement-evaluator";
    expect(() =>
      validateGate2SchemaSuccessorCohortResult(result(), rebound, manifestSha256),
    ).toThrow();
  });
  ...
  it("strict parsing rejects duplicate members, oversized input, and excessive depth", () => {
    ...
    for (const attack of [duplicate, oversized, tooDeep]) {
      expect(() =>
        parseAndValidateGate2SchemaSuccessorCohortResult(attack, manifest, manifestSha256),
      ).toThrow();
    }
  });
});
