...
export function validatePersistedGitHubObjectsOperationV1(
  ...
  if (terminalIndex !== null) {
    ...
    // A fresh GET is safe only after reconciliation returns the action state
    // from `uploading_objects` to its stable `local_ready` retry stage. Unlike
    // preflight, this effect operation already owns the action-state transition,
    // so an ambiguous actor response cannot settle as an ordinary interruption.
